...
void enAes(const char* c,const char* key,char* d){
    int clen = strlen(c);
    char *p = NULL;
    strcpy(p, c);
    while(clen%16!=0){
...
void deAes(const char* c,const char* key,char* d){
    int clen = strlen(c);
    char *p = NULL;
    strcpy(p, c);
    deAes(p,clen,(char *)key);
    d = p;
}
...
JNIEXPORT jstring JNICALL Java_com_example_demo_jni_Test_AES_1ENC
  (JNIEnv *env, jclass clz, jstring ptx, jstring key){
        const char* aeskey = env->GetStringUTFChars(key,0);
        const char* p = env->GetStringUTFChars(ptx,0);
...
JNIEXPORT jstring JNICALL Java_com_example_demo_jni_Test_AES_1DEC
  (JNIEnv *env, jclass clz, jstring ctx, jstring key){
        const char* aeskey = env->GetStringUTFChars(key,0);
        const char* c = env->GetStringUTFChars(ctx,0);
        char* d = NULL;
        deAes(c,aeskey,d);
        return env->NewStringUTF(d);
  }